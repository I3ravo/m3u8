...
#################################################
web_movie = "https://movie285-hd.com/category/erotic-rate-r-18/page/4/"
web_movie = input("\n กรุณาใส่ URL :")
    ...
    for i, link in enumerate(div.find_all("a"), start=1):
        ...
        if ptype:
            ...
            if re.search('source src="(.+?)"',view_page.text):
                refer = '{uri.scheme}://{uri.netloc}'.format(uri=urlparse(purl))
                elink =  refer+re.search('source src="(.+?)"',view_page.text).group(1)
                view_page = sess.get(elink)
                if re.search('master.m3u8',view_page.text):
                    elink =  re.sub(r'master.m3u8','master.m3u8',elink)
                elif re.search('master.m3u8',view_page.text):
                    elink =  re.sub(r'master.m3u8','master.m3u8',elink)
                else:
                    elink = elink
            else:
            ...
            if pinfo =='พากย์ไทย':
                jseries['groups'][g1]['stations'].append({"name":pname,"info":pinfo,"image":ppic,"url":elink,"referer":referer,"subtitle":"https://pastebin.com/raw/cuyErKD4"}) 
            else:
            ...
            if W_M3U:
                ...
                with open(f_path+f_m3u, 'a',encoding='utf-8') as f:
                    f.write(f'#EXTINF:-1 tvg-logo="{ppic}" group-title="{fname}" ,[{pinfo}] {pname}\n')
            ...
            for i, link in enumerate((divs), start=1):
                ...
                g1 = len(jseries['groups'])-1
                if pinfo =='พากย์ไทย':
                    jseries['groups'][g1]['stations'].append({"name":ename,"info":pinfo,"image":ppic,"url":elink,"referer":referer,"subtitle":"https://pastebin.com/raw/cuyErKD4"}) 
                else:
                ...
                if W_M3U:
                    if M_f:
                        with open(f_path+f_m3u, 'w',encoding='utf-8') as f:
                            ...
                            M_f = 0
                    with open(f_path+f_m3u, 'a',encoding='utf-8') as f:
                        f.write(f'#EXTINF:-1 tvg-logo="{ppic}" group-title="อุ๋ง อุ๋ง จะดู {pname}" ,[{pinfo}] {ename}\n')
                        f.write(f'#EXTVLCOPT:http-referer={referer}\n')
                        f.write(f'{elink}\n')
                        f.close()
                if W_W3U: 
